		...
		if (isset($_POST["mesCaptura"])) {
			$mesCaptura = $_POST["mesCaptura"];
		}
			...
			<table class="table table-striped tblTransparente">
				...
				<tbody>
					...
					for ($i = 0; $i < sizeof($enlaces); $i++) {
						...
						if ($tipoarchivo == 1) {
							if ($idfisca == 4) {
								$mpsList = getMpsEnlace($conn, $iden);
							} else {
								$mpsList = getMpsEnlaceUnidad($conn, $iden, 1);
							}
						}

						if ($tipoarchivo == 4) {
							if ($idfisca == 4) {
								$mpsList = getMpsEnlaceUnidad($conn, $iden, 4);
							} else {
								$mpsList = getMpsEnlaceUnidad($conn, $iden, 4);
							}
						}
						...
						for ($j = 0; $j < sizeof($mpsList); $j++) {
							...
							if ($tipoarchivo == 1) {
								$carpetaVal = validarCarpetaCapturada($conn, $anioCaptura, $mesCaptura, $idUnidad, $idMp);
							}
							if ($tipoarchivo == 4) {
								$carpetaVal = validarLitigacionCapturada($conn, $anioCaptura, $mesCaptura, $idMp, $idfisca);
							}
						...
						if ($longitud == 0) {
							...
							for ($k = 0; $k < $longitud; $k++) {
								...
								if ($arrayCapturadas[$k] == 0) {
									$nocap++;
								}
								if ($arrayCapturadas[$k] == 1) {
									$sicap++;
								}
							}
						}
					...
					?>
						...
						<tr>
							...
							<? if ($nocap == $longitud) { ?>
								<td class="tdRowMain">
									<div class="redCol" id="circulo"></div>
								</td>
								<? } else {
								if ($sicap == $longitud) {
								...
								?>
									<td class="tdRowMain">
										<div class="verdCol" id="circulo"></div>
									</td>
									<?
									...
									?>
										<td class="tdRowMain">
											<div class="yelloCol" id="circulo"></div>
										</td>
							<?
									}
								}
							...
							if ($tipoarchivo == 4) {
								$enlacesExInfo = getInfOCarpetasEnv($conn, $iden, 4);
							}
							...
							<?
							} else {  ?> <td class="tdRowMain">
									<div class="orangecol" id="circulo"></div>
								</td> <?  }
							...
							<? if ($longitud == 0) {   ?>
								...
								<td class="tdRowMain">
									<center><button type="button" data-toggle="modal" href="#myModalEnlaceMps" onclick="verModalMpsEnlace(<? echo $iden; ?>, '<? echo $nomCompleto; ?>', <? echo $tipoarchivo; ?>);" class="btn btn-info btn-sm redondear btnCapturarTbl" disabled><span class="glyphicon glyphicon-search"></span> Ver MP´s Capturados </button></center>
								</td>

							<? } else { ?>

								<td class="tdRowMain">
									<center><button type="button" data-toggle="modal" href="#myModalEnlaceMps" onclick="verModalMpsEnlace(<? echo $iden; ?>, '<? echo $nomCompleto; ?>', <? echo $idfisca; ?>, <? echo $tipoarchivo; ?>);" class="btn btn-info btn-sm redondear btnCapturarTbl"><span class="glyphicon glyphicon-search"></span> Ver MP´s Capturados </button></center>
								</td>
