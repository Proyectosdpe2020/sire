//Obtiene información del estado de fuerza filtrando por Area de Adcripcion o nombre del mando
function getDataEstadoFuerzaTable(filtroBusqueda){
	switch (filtroBusqueda){
		case 'busqMando' :
			var mando = document.getElementById("mando").value;
	 	var mando_value = document.querySelector("#listaMandos"  + " option[value='" + mando+ "']").dataset.value;
			var idMando = document.querySelector("#listaMandos"  + " option[value='" + mando+ "']").dataset.id;		
		break;
		case 'busqAreaAdscr' :
		document.getElementById("mando").value = "";	
			var idMando = 0;
		break;
	}
	var areaAdsc = document.getElementById("areaAdsc").value;
 $('#preloaderIMG').show();
 var table = $('#grid').DataTable();
 $("#grid").hide();
 table.destroy();
 //$('#preloader').append('<label>Cargando datos...</label><div class="progress"><div class="indeterminate"></div></div>');
 $.ajax({
  type: "POST",
  dataType: 'html',
  url:"format/estadoDeFuerza/templates/table_estadoFuerza.php",
  data: "idMando="+idMando+"&filtroBusqueda="+filtroBusqueda+"&areaAdsc="+areaAdsc,
  success: function(resp){
    $('#preloaderIMG').hide();
    $("#grid").show();
    $("#grid tbody").html(resp);
    table=$('#grid').DataTable({
       retrieve: true,
      "language": {"url": "//cdn.datatables.net/plug-ins/1.10.16/i18n/Spanish.json"},
       scrollY: 400,
       select: true,
       dom: 'Blfrtip',
       lengthMenu: [[10, 25, 50, -1],
                    ['10', '25', '50', 'todo']
                   ],
       buttons: [{
        extend: 'excel',
        title: '', 
        exportOptions: {
                    columns: [0, 1, 2, 3, 4 ]
                }

       },
       {
          extend: 'pdfHtml5',
          title: '',
          messageTop: ' ',
                customize: function ( doc ) {
                    doc.content.splice( 1, 0, {
                        margin: [ 0, 0, 0, 12 ],
                        alignment: 'center',
                        image: 'data:image/png;base64,',
                        width: 500,
        																height: 80
                    } );
                },
                exportOptions: {
                    columns: [0, 1, 2, 3, 4 ]
                }
       }],
       
       "order": [], // "0" means First column and "desc" is order type; 

      } );
  }
 });
}


//Obtiene información del mando filtrando por el campo Mando
function getDataMandosdataList(){
	$.ajax({
		type:"POST",
		dataType: "html",
		url:"format/estadoDeFuerza/templates/dataMandos_list.php",
	 success: function(respuesta){
	 	$( "#listaMandos" ).html( respuesta );
	 }
	});
}

//Muestra modal Estado de Fuerza
function showModalEstadoDeFuerza(){
	$.ajax({
		type: "POST",
		dataType: "html",
		url: 'format/estadoDeFuerza/modalEstadoDeFuerza.php',
	 success: function(respuesta){
	 	$( "#contModalEstadoDeFuerza" ).html( respuesta );
	 	$('#modalEstadoDeFuerza').modal('show');
	 }
	});
}


function saveDbMando(idMando , flag){
	var nombre = document.getElementById("nombre").value;
	var ap_paterno = document.getElementById("ap_paterno").value;
	var ap_materno = document.getElementById("ap_materno").value;
	var idCargo = document.getElementById("cargo").value;
	var idFuncion = document.getElementById("funcion").value;
	var idAreaAdscripcion = document.getElementById("areaAdscripcion").value;
 if(flag == 1){ var estatusMando = document.getElementById("estatusMando").value; }else{ var estatusMando = 3; /*3 no Existe*/ }
	if(nombre != "" && ap_paterno != "" && ap_materno && idCargo != 0 && idFuncion != 0 && idAreaAdscripcion != 0 && estatusMando != 0){
		$.ajax({
			url:'format/estadoDeFuerza/inserts/insertMando.php?nombre='+nombre+'&ap_paterno='+ap_paterno+'&ap_materno='+ap_materno+'&idCargo='+idCargo+
			    '&idFuncion='+idFuncion+'&idAreaAdscripcion='+idAreaAdscripcion+'&idMando='+idMando+'&flag='+flag+'&estatusMando='+estatusMando,
		 type:'POST',
		 contentType:false,
		 processData:false,
		 cache:false
		 }).done(function(respuesta){
		 	$( "#tableEstadoFuerza" ).html( respuesta );
		 	$('#modalEstadoDeFuerza').modal('hide');
		});
	}else{
		swal("", "Faltan datos por registrar.", "warning");
	}
}

function closeModalEstadoDeFuerza(){
	$('#modalEstadoDeFuerza').modal('hide');
}

function readImage (input) {
	var archivoInput = document.getElementById('file');
	var archivoRuta = archivoInput.value;
	var extPermitidas = /(.jpg|.JPG|.png|.PNG|.jpeg|.JPEG)$/i;
	var tamano_m = function(mega){ return Math.pow(2,20) * mega; }
 //Validar formato de archivo permitido: IMAGENES
	if(!extPermitidas.exec(archivoRuta)){
		swal("", "Formato de imagen incorrecto. \n Formatos permitidos: jpg , png y jpeg", "warning");
		archivoInput.value = ''; //Para que no se almacene en buffer
	}else{
		//Validar tamaño de la imagen, si es menor a 1mb se acepta, de lo contrario mandar alerta
		if(input.files[0].size <= tamano_m(1)){
			swal("", "Tamaño de archivo permitido", "success");
			if (input.files && input.files[0]) {
				var reader = new FileReader();
				var progress = document.getElementById('progreso');
				var etiqueta = document.getElementById('etiqueta');
				//Funcion para barra de progreso
				reader.onprogress = function (ev){
					progress.value = (ev.loaded * 100) / ev.total;
					etiqueta.innerHTML = Math.round(progress.value) + "% Carga completa";
				}
				reader.onload = function (e) {
					$('#imgMando').attr('src', e.target.result); // Renderizamos la imagen
				}
				reader.readAsDataURL(input.files[0]);
				//Despues de validar y renderizar invocamos a la función para subir imagen al servidor
				//Guardamos datos de la imagen para enviarlos al servidor
				var myFormData = new FormData();
    myFormData.append('file', input.files[0]);
				saveDataIMG(myFormData); 
			}
		}else{
			swal("", "Tamaño de archivo no permitido", "warning");
		}
	}
}

function saveDataIMG(myFormData){
	$.ajax({
		url: "format/estadoDeFuerza/inserts/uploadIMG.php",
		type: 'POST',
  processData: false, // important
  contentType: false, // important
  dataType : 'json',
  data: myFormData,
	 success: function(respuesta){

		}
	});
}

//Obtiene la area de adscripcion a partir de la fiscalia/direccion/unidad seleccionada
function getAreaAdscripcion(opc_new_or_change){
	if(opc_new_or_change == 1){
		var idFiscalia = document.getElementById("fisUnid").value; //Nueva adscripcion
	}else{
		var idFiscalia = document.getElementById("cambio_fisUnid").value; //Cambio Adcripcion
	}
	$.ajax({
		type: "POST",
		dataType: "html",
		url: "format/estadoDeFuerza/templates/reload_select_areaAdscripcion.php",
		data: "idFiscalia="+idFiscalia,
	 success: function(respuesta){
	 	if(opc_new_or_change == 1){
		 	$('#areaAdscripcion').prop('disabled', false);
			 $( "#areaAdscripcion" ).html( respuesta );
			}else{
				$('#cambio_areaAdscripcion').prop('disabled', false);
			 $( "#cambio_areaAdscripcion" ).html( respuesta );
			}
		}
	});
}

//Muestra modal Estado de Fuerza para editar información
function showModalEditarMando(idMando){
	$.ajax({
		type:"POST",
		dataType: "html",
		url: "format/estadoDeFuerza/modalEstadoDeFuerza.php",
		data: "idMando="+idMando,
	 success: function(respuesta){
	 	$( "#contModalEstadoDeFuerza" ).html( respuesta );
		 $('#modalEstadoDeFuerza').modal('show');
		}
	});
}

//Muestra modal DIRECCION para editar información
function showModalDireccion(idMando, idDomicilio){
	//VALIDAR LLENADO DE CAMPOS GENERALES ANTES DE PODER AGREGAR OTRO DATO
	var validarDatosGenerales = validaDataGeneral();	
	//Si no hay campos vacios activar modal Dirección
	if(validarDatosGenerales){
  //Al agregar un mando, debemos de llevarno los DATOS GENERALES del mando a cada modal
	 jObjectDataGeneralMando = getDataGeneralMando();

		$.ajax({
			type:"POST",
		 dataType: "html",
			url:"format/estadoDeFuerza/modalDireccion.php",
			data: "idMando="+idMando+"&idDomicilio="+idDomicilio+"&jObjectDataGeneralMando="+jObjectDataGeneralMando,
	  success:function(respuesta){
	  	$( "#contModalDireccion" ).html( respuesta );
			 $('#modalEstadoDeFuerza').modal('hide');
			 $('#modalDireccion').modal('show');
			}
		});
	}else{
		swal("", "Faltan datos generales por llenar.", "warning");
	}
}

//cierra Modal Direccion
function closeModalDireccion(){
	$('#modalDireccion').modal('hide');
	$('#modalEstadoDeFuerza').modal('show');
}

//Muestra modal VEHICULOS para editar información
function showModalVehiculos(idMando , idVehiculo){
		//VALIDAR LLENADO DE CAMPOS GENERALES ANTES DE PODER AGREGAR OTRO DATO
	var validarDatosGenerales = validaDataGeneral();	

	//Si no hay campos vacios activar modal Dirección
	if(validarDatosGenerales){
  //Al agregar un mando, debemos de llevarno los DATOS GENERALES del mando a cada modal
	 jObjectDataGeneralMando = getDataGeneralMando();
	 $.ajax({
			type: "POST",
			dataType: "html",
			url: "format/estadoDeFuerza/modalVehiculos.php",
			data: "idMando="+idMando+"&idVehiculo="+idVehiculo+"&jObjectDataGeneralMando="+jObjectDataGeneralMando,
		 success:function(respuesta){
		 	$( "#contModalVehiculos" ).html( respuesta );
			 $('#modalEstadoDeFuerza').modal('hide');
			 $('#modalVehiculos').modal('show');
			}
		});
	}else{
		swal("", "Faltan datos generales por llenar.", "warning");
	}
}

//cierra Modal VEHICULOS
function closeModalVehiculos(){
	$('#modalVehiculos').modal('hide');
	$('#modalEstadoDeFuerza').modal('show');
}

//Muestra modal ARMAS para editar información
function showModalArmas(idMando, idArma){
		//VALIDAR LLENADO DE CAMPOS GENERALES ANTES DE PODER AGREGAR OTRO DATO
	var validarDatosGenerales = validaDataGeneral();	

	//Si no hay campos vacios activar modal Dirección
	if(validarDatosGenerales){
  //Al agregar un mando, debemos de llevarno los DATOS GENERALES del mando a cada modal
	 jObjectDataGeneralMando = getDataGeneralMando();
		$.ajax({
			type: "POST",
			dataType: "html",
			url: "format/estadoDeFuerza/modalArmas.php",
			data: "idMando="+idMando+"&idArma="+idArma+"&jObjectDataGeneralMando="+jObjectDataGeneralMando,
		 success:function(respuesta){
		 	$( "#contModalArmas" ).html( respuesta );
			 $('#modalEstadoDeFuerza').modal('hide');
			 $('#modalArmas').modal('show');
			}
		});
	}else{
		swal("", "Faltan datos generales por llenar.", "warning");
	}
}

//cierra Modal ARMAS
function closeModalArmas(){
	$('#modalArmas').modal('hide');
	$('#modalEstadoDeFuerza').modal('show');
}

//Muestra modal CONTACTO DE EMERGENCIA para editar información
function showModalContactoEmergencia(idMando, idContEmerg){
		//VALIDAR LLENADO DE CAMPOS GENERALES ANTES DE PODER AGREGAR OTRO DATO
	var validarDatosGenerales = validaDataGeneral();	

	//Si no hay campos vacios activar modal Dirección
	if(validarDatosGenerales){
  //Al agregar un mando, debemos de llevarno los DATOS GENERALES del mando a cada modal
	 jObjectDataGeneralMando = getDataGeneralMando();
		$.ajax({
			type: "POST",
			dataType: "html",
			url: "format/estadoDeFuerza/modalContactoEmergencia.php",
			data: "idMando="+idMando+"&idContEmerg="+idContEmerg+"&jObjectDataGeneralMando="+jObjectDataGeneralMando,
		 success:function(respuesta){
		 	$( "#contModalContactoEmergencia" ).html( respuesta );
			 $('#modalEstadoDeFuerza').modal('hide');
			 $('#modalContactoEmergencia').modal('show');
			}
		});
	}else{
		swal("", "Faltan datos generales por llenar.", "warning");
	}
}

//cierra Modal CONTACTO DE EMERGENCIA
function closeModalContactoEmergencia(){
	$('#modalContactoEmergencia').modal('hide');
	$('#modalEstadoDeFuerza').modal('show');
}

//Muestra modal ADSCRIPCIONES para editar información
function showModalAdscripciones(idMando){
	$.ajax({
		type: "POST",
		dataType: "html",
		url: "format/estadoDeFuerza/modalAdscripciones.php",
		data: "idMando="+idMando,
	 success:function(respuesta){
	 	$( "#contModalAdscripciones" ).html( respuesta );
		 $('#modalEstadoDeFuerza').modal('hide');
		 $('#modalAdscripciones').modal('show');
		}
	});
}

//cierra Modal ADSCRIPCIONES
function closeModalAdscripciones(){
	$('#modalAdscripciones').modal('hide');
	$('#modalEstadoDeFuerza').modal('show');
}

//Muestra modal FOLIOS para editar información
function showModalFolios(idMando, idFolio){
		//VALIDAR LLENADO DE CAMPOS GENERALES ANTES DE PODER AGREGAR OTRO DATO
	var validarDatosGenerales = validaDataGeneral();	

	//Si no hay campos vacios activar modal Dirección
	if(validarDatosGenerales){
  //Al agregar un mando, debemos de llevarno los DATOS GENERALES del mando a cada modal
	 jObjectDataGeneralMando = getDataGeneralMando();
		$.ajax({
			type: "POST",
			dataType: "html",
			url: "format/estadoDeFuerza/modalFolios.php",
			data: "idMando="+idMando+"&idFolio="+idFolio+"&jObjectDataGeneralMando="+jObjectDataGeneralMando,
		 success:function(respuesta){
		 	$( "#contModalFolios" ).html( respuesta );
			 $('#modalEstadoDeFuerza').modal('hide');
			 $('#modalFolios').modal('show');
			}
		});
	}else{
		swal("", "Faltan datos generales por llenar.", "warning");
	}
}

//cierra Modal FOLIOS
function closeModalFolios(){
	$('#modalFolios').modal('hide');
	$('#modalEstadoDeFuerza').modal('show');
}

//Muestra modal INCAPACIDADES para editar información
function showModalIncapacidad(idMando, idIncapacidad){
		//VALIDAR LLENADO DE CAMPOS GENERALES ANTES DE PODER AGREGAR OTRO DATO
	var validarDatosGenerales = validaDataGeneral();	

	//Si no hay campos vacios activar modal Dirección
	if(validarDatosGenerales){
  //Al agregar un mando, debemos de llevarno los DATOS GENERALES del mando a cada modal
	 jObjectDataGeneralMando = getDataGeneralMando();
		$.ajax({
			type: "POST",
			dataType: "html",
			url: "format/estadoDeFuerza/modalIncapacidad.php",
			data: "idMando="+idMando+"&idIncapacidad="+idIncapacidad+"&jObjectDataGeneralMando="+jObjectDataGeneralMando,
		 success:function(respuesta){
		 	$( "#contModalIncapacidad" ).html( respuesta );
			 $('#modalEstadoDeFuerza').modal('hide');
			 $('#modalIncapacidad').modal('show');
			}
		});
	}else{
		swal("", "Faltan datos generales por llenar.", "warning");
	}
}

//cierra Modal INCAPACIDADES
function closeModalIncapacidad(){
	$('#modalIncapacidad').modal('hide');
	$('#modalEstadoDeFuerza').modal('show');
}

function validaDataGeneral(){
	var nombre = document.getElementById("nombre").value;
	var ap_paterno = document.getElementById("ap_paterno").value;
	var ap_materno = document.getElementById("ap_materno").value;
	var cargo = document.getElementById("cargo").value;
	var funcion = document.getElementById("funcion").value;
	var fisUnid = document.getElementById("fisUnid").value;
	var areaAdscripcion = document.getElementById("areaAdscripcion").value;
	var fechaAlta = document.getElementById("fechaAlta").value;
	var fechaActualAdsc = document.getElementById("fechaActualAdsc").value;
	if ($('input[name=sexo]:radio').is(':checked')){
		var sexo = $('input[name=sexo]:checked').val();
	}else{
		var sexo = 0;
	}

//PENDIENTE: AGREGAR FECHA DE ALTAS Y FECHA ACTUAL
	if(nombre != "" && ap_paterno != "" && ap_materno != "" && sexo != 0 && cargo != 0 && funcion != 0 && fisUnid != 0 && areaAdscripcion != "NO"){
		return true;
	}else{ 	return false; }	
		
}	

//Validar campo codigo postal maximo 5 numeros
function validateCodPostal(object){
	if (object.value.length > object.maxLength){
		object.value = object.value.slice(0, object.maxLength)
	}
}

//Validar campo telefono maximo 10 numeros
function validateTelefono(object){
	if (object.value.length > object.maxLength){
		object.value = object.value.slice(0, object.maxLength)
	}
}

function saveDbDomicilio(idMando , flag , idDomicilio){
	var domicilio = document.getElementById("domicilio").value;
	var lugarOrigen = document.getElementById("lugarOrigen").value;
	var ciudad = document.getElementById("ciudad").value;
	var estadoProvincia = document.getElementById("estadoProvincia").value;
	var codigoPostal = document.getElementById("codigoPostal").value;
 var numTelefono = document.getElementById("numTelefono").value;

 //Obtenemos los datos generales del mando llamando a la funcion 
 jObjectDataGeneralMando = getDataGeneralMando();

	if(domicilio != "" && lugarOrigen != "" && ciudad != "" && estadoProvincia != "" && codigoPostal != "" && numTelefono != "" ){
		$.ajax({
			type: "POST",
		 dataType: "html",
			url:  "format/estadoDeFuerza/inserts/insertDomicilio.php",
		 data: "domicilio="+domicilio+"&lugarOrigen="+lugarOrigen+"&ciudad="+ciudad+"&estadoProvincia="+estadoProvincia+
			      "&codigoPostal="+codigoPostal+"&numTelefono="+numTelefono+"&idMando="+idMando+"&flag="+flag+"&jObjectDataGeneralMando="+jObjectDataGeneralMando+"&idDomicilio="+idDomicilio,
		 success: function(respuesta){
		 	var json = respuesta;
		 	var obj = eval("(" + json + ")");
		 	if (obj.first == "NO") { 
		 		swal("", "No se registro verifique los datos.", "warning"); 
		 	}else{
		 		if (obj.first == "SI") {
		 			var obj = eval("(" + json + ")");
		 			swal("", "Domicilio registrado exitosamente.", "success");
		 			$('#modalDireccion').modal('hide');		
		 			showModalEditarMando(obj.idLastMando);
		 		}
		 	}
		 }
		});
	}else{
		swal("", "Faltan datos por registrar.", "warning");
	}
}

function getDataGeneralMando(){
	var nombre = document.getElementById("nombre").value;
	var ap_paterno = document.getElementById("ap_paterno").value;
	var ap_materno = document.getElementById("ap_materno").value;
	var sexo = $('input[name=sexo]:checked').val();
	var idCargo = document.getElementById("cargo").value;
	var idFuncion = document.getElementById("funcion").value;
	var idFisUnid = document.getElementById("fisUnid").value;
	var idAreaAdscripcion = document.getElementById("areaAdscripcion").value;
 var idSeccion = document.getElementById("seccion").value;
 var sexo = $('input[name=sexo]:checked').val();
 var comisionado = document.getElementById("comisionado").checked;
	var fechaAlta = document.getElementById("fechaAlta").value;
	var fechaActualAdsc = document.getElementById("fechaActualAdsc").value;
	var observacionesMando = document.getElementById("observacionesMando").value;

	var dataGeneralMando = Array();
	dataGeneralMando[0] = nombre;     dataGeneralMando[4] = idCargo;          dataGeneralMando[8] = idSeccion;         dataGeneralMando[12] = observacionesMando;
	dataGeneralMando[1] = ap_paterno; dataGeneralMando[5] = idFuncion;        dataGeneralMando[9] = comisionado;
	dataGeneralMando[2] = ap_materno; dataGeneralMando[6] = idFisUnid;        dataGeneralMando[10] = fechaAlta;
	dataGeneralMando[3] = sexo;       dataGeneralMando[7] = idAreaAdscripcion;dataGeneralMando[11] = fechaActualAdsc;
	var jObjectDataGeneralMando = {};
	for(i in dataGeneralMando){
		jObjectDataGeneralMando[i] = dataGeneralMando[i];
	}
	jObjectDataGeneralMando = JSON.stringify(jObjectDataGeneralMando);

	return jObjectDataGeneralMando;
}

function deleteItemData(form, idItem, idMando){
	swal({
		title: "Estas seguro de Eliminar?",
		text: "Se eliminara por completo el registro de la Base de Datos!",
		type: "warning",
		showCancelButton: true,
		confirmButtonColor: '#DD6B55',
		confirmButtonText: 'Si. Eliminar',
		cancelButtonText: "No, Cancelar"
	},
	function(isConfirm){
		if (isConfirm) {
			$.ajax({
			type: "POST",
		 dataType: "html",
			url:  "format/estadoDeFuerza/inserts/deleteItemForm.php",
		 data: "form="+form+"&idItem="+idItem+"&idMando="+idMando,
		 success: function(respuesta){
		 	var json = respuesta;
		 	var obj = eval("(" + json + ")");
		 	if (obj.first == "NO") { 
		 		swal("", "Fallo al eliminar los datos, intente nuevamente.", "warning"); 
		 	}else{
		 		if (obj.first == "SI") {
		 			var obj = eval("(" + json + ")");
		 			swal("", "Registro eliminado.", "success");	
		 			showModalEditarMando(obj.idLastMando);
		 		}
		 	}
		 } });
		}
	});	
}

/***FUNCION PARA GUARDAR LOS DATOS DEL VEHICULO DEL MANDO***/
function saveDbVehiculo(idMando , flag , idVehiculo){
	var marcaVehi = document.getElementById("marcaVehi").value;
	var modeloVehi = document.getElementById("modeloVehi").value;
	var placasVehi = document.getElementById("placasVehi").value;
	var vehiculoActual = 1;

 //Obtenemos los datos generales del mando llamando a la funcion 
 jObjectDataGeneralMando = getDataGeneralMando();

	if(marcaVehi != "" && modeloVehi != "" && placasVehi != "" ){
		$.ajax({
			type: "POST",
		 dataType: "html",
			url:  "format/estadoDeFuerza/inserts/insertVehiculo.php",
		 data: "marcaVehi="+marcaVehi+"&modeloVehi="+modeloVehi+"&placasVehi="+placasVehi+"&vehiculoActual="+vehiculoActual+"&idMando="+idMando+"&flag="+flag+"&jObjectDataGeneralMando="+jObjectDataGeneralMando+"&idVehiculo="+idVehiculo,
		 success: function(respuesta){
		 	var json = respuesta;
		 	var obj = eval("(" + json + ")");
		 	if (obj.first == "NO") { 
		 		swal("", "No se registro verifique los datos.", "warning"); 
		 	}else{
		 		if (obj.first == "SI") {
		 			var obj = eval("(" + json + ")");
		 			swal("", "Vehiculo registrado exitosamente.", "success");
		 			$('#modalVehiculos').modal('hide');		
		 			showModalEditarMando(obj.idLastMando);
		 		}
		 	}
		 }
		});
	}else{
		swal("", "Faltan datos por registrar.", "warning");
	}
}

/***FUNCION PARA GUARDAR LOS DATOS LA ARMA DEL MANDO***/
function saveDbArma(idMando , flag , idArma){
	var cateArma = document.getElementById("cateArma").value;
	var marcaArma = document.getElementById("marcaArma").value;
	var modeloArma = document.getElementById("modeloArma").value;
	var matriculaArma = document.getElementById("matriculaArma").value;
	var calibreArma = document.getElementById("calibreArma").value;
	var folioArma = document.getElementById("folioArma").value;

 //Obtenemos los datos generales del mando llamando a la funcion 
 jObjectDataGeneralMando = getDataGeneralMando();

	if(cateArma != "" && marcaArma != "" && modeloArma != "" && matriculaArma != "" && calibreArma != "" && folioArma != ""){
		$.ajax({
			type: "POST",
		 dataType: "html",
			url:  "format/estadoDeFuerza/inserts/insertArma.php",
		 data: "cateArma="+cateArma+"&marcaArma="+marcaArma+"&modeloArma="+modeloArma+"&matriculaArma="+matriculaArma+"&calibreArma="+calibreArma+"&folioArma="+folioArma+"&idMando="+idMando+"&flag="+flag+"&jObjectDataGeneralMando="+jObjectDataGeneralMando+"&idArma="+idArma,
		 success: function(respuesta){
		 	var json = respuesta;
		 	var obj = eval("(" + json + ")");
		 	if (obj.first == "NO") { 
		 		swal("", "No se registro verifique los datos.", "warning"); 
		 	}else{
		 		if (obj.first == "SI") {
		 			var obj = eval("(" + json + ")");
		 			swal("", "Arma registrada exitosamente.", "success");
		 			$('#modalArmas').modal('hide');		
		 			showModalEditarMando(obj.idLastMando);
		 		}
		 	}
		 }
		});
	}else{
		swal("", "Faltan datos por registrar.", "warning");
	}
}

/***FUNCION PARA GUARDAR LOS DATOS DEL CONTACTO DE EMERGENCIA DEL MANDO***/
function saveDbContactoEmergencia(idMando , flag , idContEmerg){
	var nombreCtoEmrg = document.getElementById("nombreCtoEmrg").value;
	var parentesco = document.getElementById("parentesco").value;
	var telefono = document.getElementById("telefono").value;
	var direccion = document.getElementById("direccion").value;
	var ciudadEstado = document.getElementById("ciudadEstado").value;

 //Obtenemos los datos generales del mando llamando a la funcion 
 jObjectDataGeneralMando = getDataGeneralMando();

	if(nombreCtoEmrg != "" && parentesco != "" && telefono != "" && direccion != "" && ciudadEstado){
		$.ajax({
			type: "POST",
		 dataType: "html",
			url:  "format/estadoDeFuerza/inserts/insertContactoEmergencia.php",
		 data: "nombreCtoEmrg="+nombreCtoEmrg+"&parentesco="+parentesco+"&telefono="+telefono+"&direccion="+direccion+"&ciudadEstado="+ciudadEstado+"&idMando="+idMando+"&flag="+flag+"&jObjectDataGeneralMando="+jObjectDataGeneralMando+"&idContEmerg="+idContEmerg,
		 success: function(respuesta){
		 	var json = respuesta;
		 	var obj = eval("(" + json + ")");
		 	if (obj.first == "NO") { 
		 		swal("", "No se registro verifique los datos.", "warning"); 
		 	}else{
		 		if (obj.first == "SI") {
		 			var obj = eval("(" + json + ")");
		 			swal("", "Contacto de emergencia registrada exitosamente.", "success");
		 			$('#modalContactoEmergencia').modal('hide');		
		 			showModalEditarMando(obj.idLastMando);
		 		}
		 	}
		 }
		});
	}else{
		swal("", "Faltan datos por registrar.", "warning");
	}
}

/***FUNCION PARA GUARDAR LOS DATOS DE LOS FOLIOS DEL MANDO***/
function saveDbFolios(idMando , flag , idFolio){
	var seguroSocial = document.getElementById("seguroSocial").value;
	var curp = document.getElementById("curp").value;
	var cuip = document.getElementById("cuip").value;
	var folioCredencial = document.getElementById("folioCredencial").value;
	var cup = document.getElementById("cup").value;

 //Obtenemos los datos generales del mando llamando a la funcion 
 jObjectDataGeneralMando = getDataGeneralMando();

	if(seguroSocial != "" && curp != "" && cuip != "" && folioCredencial != "" && cup){
		$.ajax({
			type: "POST",
		 dataType: "html",
			url:  "format/estadoDeFuerza/inserts/insertFolios.php",
		 data: "seguroSocial="+seguroSocial+"&curp="+curp+"&cuip="+cuip+"&folioCredencial="+folioCredencial+"&cup="+cup+"&idMando="+idMando+"&flag="+flag+"&jObjectDataGeneralMando="+jObjectDataGeneralMando+"&idFolio="+idFolio,
		 success: function(respuesta){
		 	var json = respuesta;
		 	var obj = eval("(" + json + ")");
		 	if (obj.first == "NO") { 
		 		swal("", "No se registro verifique los datos.", "warning"); 
		 	}else{
		 		if (obj.first == "SI") {
		 			var obj = eval("(" + json + ")");
		 			swal("", "Folios registrados exitosamente.", "success");
		 			$('#modalFolios').modal('hide');		
		 			showModalEditarMando(obj.idLastMando);
		 		}
		 	}
		 }
		});
	}else{
		swal("", "Faltan datos por registrar.", "warning");
	}
}

/***FUNCION PARA GUARDAR LOS DATOS DE LAS INCAPACIDADES***/
function saveDbIncapacidad(idMando , flag , idIncapacidad){
	var fechaInicio = document.getElementById("fechaInicio").value;
	var fechaFin = document.getElementById("fechaFin").value;
	var motivo = document.getElementById("motivo").value;

 //Obtenemos los datos generales del mando llamando a la funcion 
 jObjectDataGeneralMando = getDataGeneralMando();

	if(fechaInicio != "" && fechaFin != "" && motivo != ""){
		$.ajax({
			type: "POST",
		 dataType: "html",
			url:  "format/estadoDeFuerza/inserts/insertIncapacidad.php",
		 data: "fechaInicio="+fechaInicio+"&fechaFin="+fechaFin+"&motivo="+motivo+"&idMando="+idMando+"&flag="+flag+"&jObjectDataGeneralMando="+jObjectDataGeneralMando+"&idIncapacidad="+idIncapacidad,
		 success: function(respuesta){
		 	var json = respuesta;
		 	var obj = eval("(" + json + ")");
		 	if (obj.first == "NO") { 
		 		swal("", "No se registro verifique los datos.", "warning"); 
		 	}else{
		 		if (obj.first == "SI") {
		 			var obj = eval("(" + json + ")");
		 			swal("", "Incapacidad registrada exitosamente.", "success");
		 			$('#modalIncapacidad').modal('hide');		
		 			showModalEditarMando(obj.idLastMando);
		 		}
		 	}
		 }
		});
	}else{
		swal("", "Faltan datos por registrar."+fechaInicio, "warning");
	}
}

/***FUNCION PARA GUARDAR LOS DATOS DEL MANDO***/
function saveDataMando(idMando , flag){
	//VALIDAR LLENADO DE CAMPOS GENERALES ANTES DE PODER GUARDAR
	var validarDatosGenerales = validaDataGeneral();	

	if(validarDatosGenerales){
		//Al agregar un mando, debemos de llevarno los DATOS GENERALES del mando a cada modal
	 jObjectDataGeneralMando = getDataGeneralMando();
		$.ajax({
			type: "POST",
		 dataType: "html",
			url:  "format/estadoDeFuerza/inserts/insertDataMando.php",
		 data: "idMando="+idMando+"&flag="+flag+"&jObjectDataGeneralMando="+jObjectDataGeneralMando,
		 success: function(respuesta){
		 	var json = respuesta;
		 	var obj = eval("(" + json + ")");
		 	if (obj.first == "NO") { 
		 		swal("", "No se registro verifique los datos.", "warning"); 
		 	}else{
		 		if (obj.first == "SI") {
		 			var obj = eval("(" + json + ")");
		 			swal("", "Información registrada exitosamente.", "success");	
		 			showModalEditarMando(obj.idLastMando);
		 		}
		 	}
		 }
		});
	}else{
		swal("", "Faltan datos por registrar.", "warning");
	}
}

/***FUNCION PARA ELIMINAR LA FOTO DEL MANDO***/
function deleteFoto(idMando , idFoto, srcFoto){
		$.ajax({
			type: "POST",
		 dataType: "html",
			url:  "format/estadoDeFuerza/inserts/deleteFoto.php",
		 data: "idMando="+idMando+"&idFoto="+idFoto+"&srcFoto="+srcFoto,
		 success: function(respuesta){
		 	var json = respuesta;
		 	var obj = eval("(" + json + ")");
		 	if (obj.first == "NO") { 
		 		swal("", "Ha ocurrido un error al intentar eliminar la imagen.", "warning"); 
		 	}else{
		 		if (obj.first == "SI") {
		 			var obj = eval("(" + json + ")");
		 			swal("", "imagen eliminda correctamente.", "success");	
		 			showModalEditarMando(obj.idLastMando);
		 		}
		 	}
		 }
		});
}

function saveDbAdscripcion(idMando , flag){
	var cambio_fisUnid = document.getElementById("cambio_fisUnid").value;
	var cambio_areaAdscripcion = document.getElementById("cambio_areaAdscripcion").value;
	var cambio_seccion = document.getElementById("cambio_seccion").value;
 var fechaCambioAdscrip = document.getElementById("fechaCambioAdscrip").value;
 var cambio_estatusMando = document.getElementById("cambio_estatusMando").value;

	if(cambio_fisUnid != "" && cambio_areaAdscripcion != "" && cambio_seccion != "" && fechaCambioAdscrip != "" && cambio_estatusMando != 0 ){
		$.ajax({
			type: "POST",
		 dataType: "html",
			url:  "format/estadoDeFuerza/inserts/insertcambioAsdcripcion.php",
		 data: "cambio_fisUnid="+cambio_fisUnid+"&cambio_areaAdscripcion="+cambio_areaAdscripcion+"&cambio_seccion="+cambio_seccion+"&fechaCambioAdscrip="+fechaCambioAdscrip+
		       "&cambio_estatusMando="+cambio_estatusMando+"&idMando="+idMando+"&flag="+flag,
		 success: function(respuesta){
		 	var json = respuesta;
		 	var obj = eval("(" + json + ")");
		 	if (obj.first == "NO") { 
		 		swal("", "No se registro verifique los datos.", "warning"); 
		 	}else{
		 		if (obj.first == "SI") {
		 			var obj = eval("(" + json + ")");
		 			swal("", "Cambio de adscripción registrado exitosamente.", "success");
		 			$('#modalAdscripciones').modal('hide');		
		 			showModalEditarMando(obj.idLastMando);
		 		}
		 	}
		 }
		});
	}else{
		swal("", "Faltan datos por registrar.", "warning");
	}
}

function generarFormatoAdscripcion(idMando){
	//Obtenemos los datos generales del mando llamando a la funcion 
 jObjectDataGeneralMando = getDataGeneralMando();
 $.ajax({
  type: "POST",
  dataType: 'json',
  url: 'format/estadoDeFuerza/documents/word/generarWorld.php',
  data: "jObjectDataGeneralMando="+jObjectDataGeneralMando+"&idMando="+idMando,
 }).done(function(data){
    var $a = $("<a>");
    $a.attr("href",data.file);
    $("body").append($a);
    $a.attr("download","cambioAdscripcion.docx");
    $a[0].click();
    $a.remove();
});

}

function generarFormatoPrimerAdscrip(idMando){
	//Obtenemos los datos generales del mando llamando a la funcion 
 jObjectDataGeneralMando = getDataGeneralMando();
 $.ajax({
  type: "POST",
  dataType: 'json',
  url: 'format/estadoDeFuerza/documents/word/formatoPrimerAdscripcion.php',
  data: "jObjectDataGeneralMando="+jObjectDataGeneralMando+"&idMando="+idMando,
 }).done(function(data){
    var $a = $("<a>");
    $a.attr("href",data.file);
    $("body").append($a);
    $a.attr("download","asignarAdscripcion.docx");
    $a[0].click();
    $a.remove();
});

}

function descargarReporteGeneral(idMando){
 $.ajax({
  type: "POST",
  dataType: 'json',
  url: 'format/estadoDeFuerza/documents/word/formatoInformacionGeneral.php',
  data: "idMando="+idMando,
 }).done(function(data){
    var $a = $("<a>");
    $a.attr("href",data.file);
    $("body").append($a);
    $a.attr("download","informacionGeneral.docx");
    $a[0].click();
    $a.remove();
});
}

function descargarReporteCompleto(idMando){
 $.ajax({
  type: "POST",
  dataType: 'json',
  url: 'format/estadoDeFuerza/documents/word/formatoInformacionCompleta.php',
  data: "idMando="+idMando,
 }).done(function(data){
    var $a = $("<a>");
    $a.attr("href",data.file);
    $("body").append($a);
    $a.attr("download","informacionGeneral.docx");
    $a[0].click();
    $a.remove();
});
}