...
if (isset($_POST['nombreCtoEmrg'])){ $nombreCtoEmrg = $_POST['nombreCtoEmrg']; } 
if (isset($_POST['parentesco'])){ $parentesco = $_POST['parentesco']; } 
if (isset($_POST['telefono'])){ $telefono = $_POST['telefono']; }
if (isset($_POST['direccion'])){ $direccion = $_POST['direccion']; }
if (isset($_POST['ciudadEstado'])){ $ciudadEstado = $_POST['ciudadEstado']; }

if($data[9] == false){ $data[9] = 0; } else { $data[9] = 1; } //data[9] obtiene si es comisionado

//Si $idMando == 0 -> indica el registro de un nuevo mando ya  el $idMando y se cre un nuevo registro en la bd
if($idMando == 0){
 $queryTransaction = " 
                      BEGIN                     
                            BEGIN TRY 
                              BEGIN TRANSACTION
                                    SET NOCOUNT ON 
                                      ...
                                      INSERT INTO estadoFuerza.contactosEmergencia (idMando, nombre, parentesco, telefono, direccion, ciudad) VALUES(@insertado, '$nombreCtoEmrg', '$parentesco', '$telefono', '$direccion', '$ciudadEstado')
 ...
 if($idContEmerg != 0){
                             ...
                             BEGIN TRY 
                               BEGIN TRANSACTION
                                     SET NOCOUNT ON 
                                       ...
                                       UPDATE estadoFuerza.contactosEmergencia SET 
                                              nombre = '$nombreCtoEmrg',
                                              parentesco = '$parentesco',
                                              telefono = '$telefono',
                                              direccion = '$direccion',
                                              ciudad = '$ciudadEstado'
                                              WHERE idContEmerg = $idContEmerg
  ...
  $queryTransaction = " INSERT INTO estadoFuerza.contactosEmergencia (idMando, nombre, parentesco, telefono, direccion, ciudad) VALUES($idMando, '$nombreCtoEmrg', '$parentesco', '$telefono', '$direccion', '$ciudadEstado')";
}
