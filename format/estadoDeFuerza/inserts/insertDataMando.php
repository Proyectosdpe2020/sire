...
if($idMando == 0){
 ...
 if ($result) {
  ...
  if(isset($_SESSION['tmp_path'])){
   ...
   if( copy($_SESSION['tmp_path'], $carpeta_destino.$idMando.".".$extension) ){
    //Comprobamos si ya tiene una foto almacenada en la BD para comprobar si sera insercion o actualizacion de la misma
    $getDataFotoTrue = getDataFotoTrue($conn, $idMando);
    if($getDataFotoTrue){
      unlink($carpeta_destino.$getDataFotoTrue[0][1]);
      $insertFoto = "UPDATE estadoFuerza.fotografias SET foto = '$idMando.$extension' where idMando = $idMando";
    }else{
     $insertFoto = "INSERT INTO estadoFuerza.fotografias (idMando, foto) VALUES($idMando, '$idMando.$extension')"; 
    } 
    $resultInsertF = sqlsrv_query($conn, $insertFoto, array(), array( "Scrollable" => 'static' ));
    unlink($_SESSION['tmp_path']);
    unset($_SESSION['tmp_path']);
