...
<div id="review_modal_buttons" style="display: inline-block; padding: 10px; width: 100%;">
    ...
    <button type="button" class="btn btn-danger" style="padding: 10px; width: 33%; margin: 0 auto;" onclick="reviewReport(<?php echo $_GET['period'].', '.$_GET['year'].', '.$_GET['file'].', '.$_GET['link'];; ?>, 'rec')">Rechazar <i class="fa fa-times" aria-hidden="true"></i></button>
    <button type="button" class="btn btn-light" style="padding: 10px; width: 33%; float: left;" onclick="closeReviewReport()">Cancelar</button>
